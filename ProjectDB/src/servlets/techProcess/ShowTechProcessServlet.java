...
@WebServlet("/showTechProc")
public class ShowTechProcessServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		em.getTransaction();
		try {
			TechProcess techProc = new TechProcessService(em).find(techProcId);
			request.setAttribute("techProc", techProc);
		} catch (SQLException e) {e.printStackTrace();}
		finally{em.close();}
		request.getServletContext().getRequestDispatcher("/WEB-INF/views/techProcess/showTechProcessView.jsp").forward(request, response);
	}
