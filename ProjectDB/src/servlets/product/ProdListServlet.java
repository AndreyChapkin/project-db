	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		EntityManager em = emf.createEntityManager();
			...
			try{
				...
				List<Product> prodList = new ProductService(em).getAll();
				em.getTransaction().commit();
				request.setAttribute("prodList", prodList);
			}
			catch(Exception e) {response.getWriter().println(e);}
			finally {em.close();}
			request.getServletContext().getRequestDispatcher("/WEB-INF/views/product/prodListView.jsp").forward(request, response);
	}
